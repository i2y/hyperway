...
func main() {
	// Create a service with all protocols enabled
	svc := rpc.NewService("CalculatorService",
		rpc.WithPackage("calculator.v1"),
		rpc.WithValidation(true),
		rpc.WithPreset(rpc.PresetAll),       // Enable all protocols
		rpc.WithJSONRPC("/api/jsonrpc", 50), // Customize JSON-RPC settings
